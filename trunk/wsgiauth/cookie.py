...
::
  ...
  >>> # from paste.auth.cookie import Cookie
...
def get_cookies(environ):
    ...
    if environ.has_key('paste.cookies'):
        ...
        if check_header == header: return cookies
...
class AuthCookieSigner(object):
    ...
    def sign(self, content):
        ...
        if len(cookie) > self.maxlen: raise CookieTooLarge(content, cookie)
        return cookie
    ...
    def auth(self,cookie):
        ...
        '''
        decode = base64.b64decode(cookie.replace('_', '/').replace('~', '='))
...
class Cookie(object):
    ...
    def __init__(self, application, cookie_name=None, scanlist=None,
        ...
        if not signer: signer = self.signer_class(secret, timeout, maxlen)
        self.signer = signer
    ...
    def __call__(self, environ, start_response):
        ...
        if jar.has_key(self.cookie_name):
            ...
            if content:
                for pair in content.split(';'):
                    k, v = pair.split('=')
                    k = decode(k)
                    if k not in scanlist: scanlist.append(k)
                    if k in environ: continue
                    environ[k] = decode(v)
                    if 'REMOTE_USER' == k: environ['AUTH_TYPE'] = 'cookie'
        ...
        def response_hook(status, response_headers, exc_info=None):
            ...
            if content:
                ...
                if 'https' == environ['wsgi.url_scheme']: cookie += ' secure;'
...
def cookie(realm, authfunc):    
    def decorator(application):
        return Cookie(application, realm, authfunc)
    return decorator

__all__ = ['AuthCookieSigner', 'AuthCookieEnviron', 'Cookie', 'cookie']